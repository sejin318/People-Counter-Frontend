...
import { setCanvas, drawRegion, resetCanvas } from '../actions/Image';
  ...
  render() {
    ...
    return (
      <div>
        ...
        {buttons.map((data) => (
          <Button onClick={() => resetCanvas(canvas, this.refs.image).then(drawRegion(canvas, regions[data]))} style={{ marginLeft : 20 }} variant="contained" color="tertiary">
            {data}
          </Button>
        ))}
      </div>
